    ...
    for arn in tagged_arns:
        if ":oidc-provider/" in arn:
            doer.run_allow_fail(
                ...
                ["iam", "delete-open-id-connect-provider", "--open-id-connect-provider-arn", arn],
            )
...
def iam_cleanup_ephemeral_roles(doer: Doer) -> None:
    ...
    Clean up ephemeral roles/instance-profiles first, then policy deletion becomes possible.
    """
    ctx = doer.ctx
        ...
        for role in prof.get("Roles", []) or []:
            ...
            if rname:
                doer.run_allow_fail(
                    ...
                    ["iam", "remove-role-from-instance-profile", "--instance-profile-name", name, "--role-name", rname],
                )
        doer.run_allow_fail(
    ...
    for role in roles.get("Roles", []) or []:
        ...
        attached = doer.aws.json(["iam", "list-attached-role-policies", "--role-name", name]) or {}
        for ap in attached.get("AttachedPolicies", []) or []:
            arn = ap.get("PolicyArn")
            if arn:
                doer.run_allow_fail(
                    f"iam detach role policy {name} {arn}",
                    ["iam", "detach-role-policy", "--role-name", name, "--policy-arn", arn],
                )
        ...
        for pname in inline.get("PolicyNames", []) or []:
            doer.run_allow_fail(
                ...
                ["iam", "delete-role-policy", "--role-name", name, "--policy-name", pname],
            )
